...
class GLMLLMAPI:
    ...
    """
    def __init__(self):
        # 读取配置文件，容错匹配 LLM 下的提供方（优先“智谱官方”，否则回退到第一个含 model 列表的提供方）
        config_path = os.path.join(os.path.dirname(__file__), '..', 'config.json')
        with open(config_path, 'r', encoding='utf-8') as f:
            cfg_all = json.load(f)
        llm = cfg_all.get('LLM', {}) or {}
        provider_key = None
        if "智谱官方" in llm and isinstance(llm["智谱官方"], dict):
            provider_key = "智谱官方"
        else:
        ...
        if "智谱官方" in llm and isinstance(llm["智谱官方"], dict):
            ...
            for k, v in llm.items():
                if isinstance(v, dict) and isinstance(v.get('model'), list):
                    selected = v
                    break
        model_options = selected.get('model', ['glm-4-flash-250414'])
        return {
            "required": {
    ...
    def infer(self, model, max_tokens, temperature, top_p, system_prompt, user_prompt):
        ...
        if not api_key:
            return ("", "错误：未配置GLM API Key，请在config.json的 LLM 部分对应提供方下设置 api_key（例如“智谱官方”.api_key）", "")
